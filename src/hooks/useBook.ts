...
import type { Book, FeaturedCollectionData } from "@/constant/interfaces";
...
export const useBook = () => {
  ...
  const getTopSellingBooks = async () => {
    ...
    return response
      .filter((book) => book.quantity_sold !== undefined)
      .slice(0, 10);
  };
  ...
  const getBookFeaturedCollections = async () => {
    return await Request.get<FeaturedCollectionData[]>(
